...
export default function AdminPage() {
  ...
  const handleAddLocation = async () => {
    const { value: formValues } = await Swal.fire({
      title:
        '<span style="font-family:Mali; font-weight:900; color:#1e1b4b;">✨ เพิ่มพิกัดใหม่</span>',
      background: "#ffffff",
      borderRadius: "30px",
  ...
  const handleEditLocation = async (loc) => {
    const currentTypeKey =
      Object.keys(typeMapping).find(
        (key) => typeMapping[key].label === loc.type,
      ) || "green";
    const { value: formValues } = await Swal.fire({
      title:
        '<span style="font-family:Mali; font-weight:900; color:#1e1b4b;">✏️ แก้ไขข้อมูลสถานที่</span>',
      background: "#ffffff",
      borderRadius: "30px",
      showCloseButton: true,
  ...
  const handleToggleBan = (user) => {
    ...
    Swal.fire({
      ...
      text: isBanning
        ? `ผู้ใช้ ${user.name} จะไม่สามารถใช้งานระบบได้ชั่วคราว`
        : `คืนสิทธิ์การใช้งานให้คุณ ${user.name}`,
      icon: "warning",
      showCancelButton: true,
    ...
    if (reply) {
      ...
      setContactRequests(
        contactRequests.map((c) =>
          c.id === req.id ? { ...c, status: "resolved" } : c,
        ),
      );
      Swal.fire({
        icon: "success",
          ...
          >
            <button
              ...
              style={{
                ...
                border: "none",
                background:
                  activeTab === "locations" ? "#EEF2FF" : "transparent",
                color: activeTab === "locations" ? "#6D28D9" : "#64748B",
                fontWeight: activeTab === "locations" ? 700 : 600,
              }}
            >
            ...
            <button
              ...
              style={{
                ...
                cursor: "pointer",
                border: "none",
                background: activeTab === "contact" ? "#EEF2FF" : "transparent",
                color: activeTab === "contact" ? "#6D28D9" : "#64748B",
      ...
      <main style={{ flex: 1, padding: "40px 80px" }}>
        ...
        >
          <div style={{ position: "relative" }}>
            ...
            >
              {activeTab === "locations"
                ? `พบทั้งหมด ${locations.length} พิกัดในระบบ`
                : activeTab === "users"
                  ? `มีผู้ใช้งานทั้งหมด ${users.length} ราย`
                  : `รายการคำร้องทั้งหมด ${contactRequests.length} เรื่อง`}
            </p>
          </div>
          {activeTab === "locations" && (
            <button
          ...
          >
            <table style={{ width: "100%", borderCollapse: "collapse" }}>
              <thead>
                ...
                >
                  <th style={{ padding: "20px 24px", color: "#64748B" }}>
                    ผู้ใช้งาน
                  </th>
                  <th style={{ padding: "20px 24px", color: "#64748B" }}>
                    สถานะ
                    ...
                    <td style={{ padding: "20px 24px" }}>
                      <span
                        style={{
                          ...
                          color:
                            user.status === "active" ? "#16A34A" : "#EF4444",
                        }}
                      >
                      ...
                      <button
                        ...
                        onMouseEnter={(e) =>
                          (e.target.style.background =
                            user.status === "active" ? "#FEE2E2" : "#DCFCE7")
                        }
                        onMouseLeave={(e) =>
        ...
        activeTab === "contact" ? (
          ...
          >
            ...
            <p style={{ color: "#64748B", marginBottom: "18px" }}>
              จัดการข้อความจากผู้ใช้งาน — ตอบกลับหรือปิดคำร้องได้
            </p>
                ...
                >
                  <div>
                    ...
                    >
                      ...
                      <strong
                        style={{
                          color: req.status === "open" ? "#16A34A" : "#6B7280",
                        }}
                      >
                        {req.status}
                      </strong>
                  ...
                  >
                    <button
                      onClick={() => handleReplyRequest(req)}
                      style={{
                        padding: "8px 12px",
                        borderRadius: "10px",
                        background: "#4F46E5",
                    ...
                    <button
                      onClick={() => handleToggleResolve(req.id)}
                      style={{
                        padding: "8px 12px",
                        borderRadius: "10px",
                        background:
                          req.status === "open" ? "#10B981" : "#F3F4F6",
                        color: req.status === "open" ? "white" : "#374151",
                        border: "none",
                        cursor: "pointer",
                        fontWeight: 700,
                      }}
                    >
                      {req.status === "open" ? "ดำเนินการแล้ว" : "เปิดใหม่"}
                    </button>
                    <button
        ...
        >
          ...
          <div className="profile-modal-card">
            ...
            <div className="avatar-modal-section">
              <div
                className={`avatar-modal-wrapper ${isEditingProfile ? "editable" : ""}`}
                onClick={() =>
                  isEditingProfile && fileInputRef.current?.click()
                }
              >
                <img
                  src={editProfile.profileImage || "/avatar-placeholder.png"}
                  alt="Profile"
                />
                {isEditingProfile && (
              ...
              >
                {editProfile.firstName || user?.firstName || "User"}{" "}
            ...
            >
              <div className="field">
                ...
                <div
                  className={`input-box ${isEditingProfile ? "editing" : ""}`}
                >
            ...
            <div className="field">
              ...
              <div
                className="input-box"
                style={{ opacity: 0.8, backgroundColor: "#EDF2F7" }}
              >
            ...
            {!isEditingProfile ? (
              ...
              <div className="btn-modal-group">
                ...
                <button
                  ...
                  className="btn-modal btn-modal-secondary"
                  style={{ color: "#E11D48", fontFamily: "'Mali', sans-serif" }}
                >
                  ยกเลิก
                </button>
              </div>
