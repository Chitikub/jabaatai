...
import { Heart, ChevronRight, Calendar, Trash2, MapPin } from 'lucide-react';
import Navbar from '../components/Navbar'; // 👈 เช็ค Path ให้ตรงกับโฟลเดอร์ของคุณ
    ...
    <>
      ...
      <main className="page-container">
        <style>{`
          ...
          .del-btn:hover { background: #FEF2F2; transform: scale(1.15); }
        ...
        <div className="glass-card">
          ...
          <aside className="sidebar">
            ...
            <div style={{ marginTop: '20px' }}>
              <div style={{ textAlign: 'center', background: 'rgba(255,255,255,0.7)', padding: '25px', borderRadius: '24px', border: '1px solid #E2E8F0', boxShadow: '0 4px 15px rgba(0,0,0,0.02)' }}>
                <div style={{ fontSize: '2.5rem', fontWeight: '900', color: '#0F172A', lineHeight: '1' }}>
                  {favorites.length}
                </div>
                <p style={{ fontSize: '0.85rem', color: '#64748B', fontWeight: '700', textTransform: 'uppercase', margin: '5px 0 0', letterSpacing: '1px' }}>
                  สถานที่โปรด
                </p>
              </div>
            </div>
          ...
          <section className="main-content">
            ...
            <div style={{ display: 'flex', justifyContent: 'center' }}>
              <div className="tab-switcher">
                <button className="inactive-tab" onClick={() => router.push('/history')}>ประวัติการใช้งาน</button>
                <button className="active-tab">รายการโปรด</button>
              </div>
            </div>
            ...
            <div className="list-scroll">
              {favorites.length > 0 ? favorites.map((item) => (
                <div key={item.id} className="item-card" onClick={() => router.push(`/location/${item.id}`)}>
                  <div style={{ display: 'flex', alignItems: 'center' }}>
                    ...
                    <div>
                      ...
                      <div style={{ display: 'flex', gap: '15px', marginTop: '6px' }}>
                        ...
                        <span style={{ fontSize: '0.85rem', color: '#64748B', display: 'flex', alignItems: 'center', gap: '5px' }}>
                          <MapPin size={14}/> {item.type === 'forest' ? 'ธรรมชาติ' : item.type === 'cafe' ? 'คาเฟ่' : 'ทะเล'}
                        </span>
                      </div>
                    </div>
                  </div>
                ...
                <div style={{ textAlign: 'center', padding: '80px 0', color: '#94A3B8' }}>
                  <Heart size={64} color="#E2E8F0" strokeWidth={1.5} style={{ margin: '0 auto 20px' }} />
                  <p style={{ fontSize: '1.1rem', fontWeight: '600', color: '#475569' }}>ยังไม่มีรายการโปรด</p>
                  <p style={{ fontSize: '0.95rem', marginTop: '8px' }}>กดหัวใจสถานที่ที่ชอบ แล้วกลับมาดูที่นี่ได้เลย!</p>
                </div>
              )}
            </div>
          </section>
